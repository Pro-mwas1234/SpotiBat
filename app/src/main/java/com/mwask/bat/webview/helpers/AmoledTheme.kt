...
fun buildAmoledJs(enabled: Boolean): String {
    return if (enabled) {
        """
            (function(){
                var aled = document.getElementById('spotiBat-amoled-theme') || document.createElement('style');
                aled.id = 'spotiBat-amoled-theme';
        ...
        """
            (function(){
                var aled = document.getElementById('spotiBat-amoled-theme');
                if (aled) aled.remove();
            })();
        """.trimIndent()
    }
